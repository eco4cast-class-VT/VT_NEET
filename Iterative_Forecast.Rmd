...
$$NEE(t) = \beta_1 \times NEE(t-1) + \beta_2 \times \Delta SW_{incoming} + \beta_3 \times \Delta temp$$
...
TempDLM <- nimbleCode({
  ...
  for(t in 2:n){
    pred[t] <- b1 * x[t - 1] + b2 * (inSW[t] - inSW[t - 1]) + b3 * (temp[t] - temp[t - 1])
...
nimble_burn_TempDLM_KONZ = PlotChain(siteID = "OSBS", nimble_out_TempDLM_OSBS)
nimble_burn_TempDLM_OSBS = PlotChain(siteID = "KONZ", nimble_out_TempDLM_KONZ)
...
Since the upper confidence interval (C.I.) for the Brooks-Gelman-Rubin convergence diagnostic is less than 1.1 for all parameters, we can conclude the chains have converged for BART.
...
Since the upper confidence interval (C.I.) for the Brooks-Gelman-Rubin convergence diagnostic is less than 1.1 for all parameters, we can conclude the chains have converged for KONZ.
...
Since the upper confidence interval (C.I.) for the Brooks-Gelman-Rubin convergence diagnostic is less than 1.1 for all parameters, we can conclude the chains have converged for OSBS.
...
Since the upper confidence interval (C.I.) for the Brooks-Gelman-Rubin convergence diagnostic is less than 1.1 for all parameters, we can conclude the chains have converged for SRER.
...
chain_TempDLM_BART <- nimble_burn_TempDLM_BART %>%
  spread_draws(y[day], x[day], b1, b2, b3, sd_add)
...
# GetChainStats <- function(chain_TempDLM)
# {
...
# }
# 
# plot_chain_TempDLM_SRER = GetChainStats(chain_TempDLM_BART)
...
# 
# plot_chain_TempDLM_SRER = GetChainStats(chain_TempDLM_SRER)
...
for(m in 1:ensemble_size){
  ...
  for(t in 1:forecast_length){
    ...
    if(t > 1){  # Condition set b/c initial condition already specified for state variable
      ...
      pred <- b1[m] * NEE_state[t - 1, m] + b2[m] * (forecast_inSW[t, m] - forecast_inSW[t - 1, m]) +  b3[m] * (forecast_temp[t, m] - forecast_temp[t - 1, m])
...
PlotObsAndForecast <- function(siteID, nee_forecast, plot_chain_TempDLM, target_data_sub)
{
  ...
  title = paste("Site:", siteID, "| Time Period: ", substr(plot_chain_TempDLM$time[1], 1, 10), "to",
  ...
  plot = ggplot() +
    geom_line(data = plot_chain_TempDLM, aes(x = time, y = mean, color = "1")) +
    geom_line(data = plot_NEE_forecast, aes(x = time, y = mean, color = "2")) +
    ...
    geom_point(data = target_data_sub, aes(x = time, y = nee), color = "black") +
    scale_fill_manual(name = "Period", values = c("gray", plot_colors[4]),
                      labels = c("Historical", "Forecast")) +
    scale_color_manual(name = "Period", values = c("gray", plot_colors[4]),
                       labels = c("Historical", "Forecast")) +
